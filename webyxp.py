...
def yxpTimeGet():
    ...
    time_json = json.loads(out.text)
    return time_json["recordset"]["system_time"]
...
def yxpSubject(uid):
    ...
    apiS = 'https://www.anoah.com/api/?q=json/ebag5/Classes/getClassSubject&info={"class_id":"' + class_id + '"}'
    subjectInfoList = requests.get(apiS).json()['recordset']
    rejson = {}
    for subject in subjectInfoList:
        rejson[subject['subject_name']] = subject['edu_subject_id']
    return rejson
####################################################### 
...
#######################################################
    ...
    if arg[1]=="yxpRs":
        ...
        if(arg[3]=="最新"):
            # subjectJson = {"语文":1,"数学":2,"英语":3,"化学":4,"历史":5,"地理":6,"生物":7,"物理":8,"美术":32,"信息":33,"音乐":14,"体育":23,"道法":437}
            subjectJson = yxpSubject(arg[2])
            # print(subjectJson)
            # 改变算法,获取全部学科
            #TODO 重构
            subjectidlist=[]
            ...
            for i in subjectJson:
                subjectNamelist.append(i)
                subjectidlist.append(subjectJson[i])
            ...
            for i in range(len(subjectNamelist)):
                url="http://e.anoah.com/api/?q=json/ebag5/Statistics/getStudentScoreInfo&info="\
                    "{\"user_id\":%s,\"class_id\":\"%s\",\"type\":0,\"subject_id\":%s,\"pagesize\":1,\"page\":1,\"start_date\":\"\",\"end_date\":\"\"}&pmatsemit=%s"%\
                        (arg[2],Classid,subjectidlist[i],yxpTimeGet())
